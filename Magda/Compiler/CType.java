...
public class CType extends Vector<ITypeElement>
{   boolean isAll=false;
    ...
    public CType(CEnvironment env, CPolymorphismParam param, CType value)
    { super();
    ...
    public CType(CEnvironment env, ITypeElement aTypeElement) //
    { super();
      app = new CPolyApplicationValues();
      add (env.getMixin("Object"));
      add (aTypeElement);
	...
	{ return isSubTypeOf(env, other) &&
 		      other.isSubTypeOf(env, this);
	}
            ...
            { CMixinDeclaration decl =  (CMixinDeclaration) get(i);
              ...
              { CMixinDeclaration baseMixin = (CMixinDeclaration) baseType.get(j);
                if (!MixinExistsInPrefix(i-1, baseMixin))
                   throw new CTypeError("Error in Expression used to create new object from: "+this+" : "+
                             decl.MixinName+" requires "+baseMixin.MixinName+ " which is not present");
              }
            
            }
            else
              throw new Error("instantiation from the polymorphic params not yet supported!");
            //TODO: czy tu jakoœ powinnismy polimorficzne parametry obsluzyc???? moze ich boundy posprawdzac
          }
        
        }

	public CType setPolyParamsFrom (CPolyApplicationValues vals)
	{ CType res = new CType(false);
	  for (int i=0; i<size(); i++)
	    if (get(i) instanceof CPolymorphismParam)
