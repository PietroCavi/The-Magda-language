...
public class COverrideMethodDeclaration implements IMethodDeclaration
{
	...
	public COverrideMethodDeclaration (IMixinExpression aResType, String aMixinName, String aMethodName, CParameterDeclarations aPars, CMethodBody aBody, int aLineNo, String aProgramFile)
	...
	{ CTypeError.LineNo = LineNo;
	  ...
	  Pars.CheckTypes(env);
      //
	  if (!GetSourceMethod(env).getResultType(env).isIsomorphicTo(env, 
		      getResultType(env)
		 ))
		 throw new CTypeError ("Result type differs in overriding method " + MethodName);
	  //
	  if (!env.getMixin(MixinName).getNewMethod(MethodName).getFormalParameters().isIsomorphicTo(
		      ContainerMixin.GetType(env), env, getFormalParameters()
		 ))
